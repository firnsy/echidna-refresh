package Echidna::Service::Database::DBI;

use strict;
use 5.010;

use base qw(Echidna::Service::Database::Base);
...
sub fire_watcher {
    ...
    $w = AE::timer 1, $time, sub {
       say  "Running: " . (keys %{$self->{__running}}). " (".join(", ", (keys %{$self->{__running}})). ")";
    };
}
...
sub fetch {
    ...
    if ($self->{__debug}) {
        say  "Running: " .(keys %{$self->{__running}})
