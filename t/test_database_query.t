...
use_ok 'Echidna::Database::Query::SQLRead';

my $query = Echidna::Database::Query::SQLRead->new;
isa_ok($query, 'Echidna::Database::Query::SQLRead');
...
ok( $sql ~~ $query->render, 'SQLRead Str');
ok([1, 20, 10, 5] ~~ $query->params, 'PARAMS Check');

my $query2 = Echidna::Database::Query::SQLRead->new;
$query2->table('event')
       ->offset('2')
       ->limit(10);

ok('SELECT * FROM event  LIMIT 10 OFFSET 2' eq $query2->render, 'SQLRead Query Check 2');
